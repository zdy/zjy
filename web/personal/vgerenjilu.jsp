...
<div class="container-fluid">
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
        ...
        <div style="margin-top: 30px;"></div>
                <c:choose>
                    <c:when test="${sessionScope.volunteer != null}">
                        <div class="frame-block" style="margin-top: 100px">
                            <span style="font-size: 30px;font-weight: bold">照片:</span>
                            <c:choose>
                                <c:when test="${sessionScope.volunteer.picture != null}">
                                    <img src="${sessionScope.volunteer.picture}" alt="" style="width: 150px;height: 200px;">
                                </c:when>
                                <c:otherwise>
                                    <img src="${sessionScope.volunteer.picture}" alt="" style="width: 100px;height: 100px;border: none">
                                </c:otherwise>
                            </c:choose>
                        </div>
                        <br>
                        <p style="font-weight: bold;font-size: 22px">信息如下:</p>
                        <table class="table table-bordered table-hover definewidth m10">
                            ...
                            <tr>
                                <td><c:out value="${sessionScope.volunteer.name }"></c:out>
                                </td>
                                <td><c:out value="${sessionScope.volunteer.age }"></c:out>
                                </td>
                                <td><c:out value="${sessionScope.volunteer.sex }"></c:out>
                                </td>
                                <td>
                                    <c:out value="${sessionScope.volunteer.uid}"></c:out>
                                </td>
                                <td><c:out value="${sessionScope.volunteer.year}"></c:out></td>
                                <td><c:out value="${sessionScope.volunteer.motto }"></c:out></td>
                                <td><c:out value="${sessionScope.volunteer.phone }"></c:out></td>
                                <td><c:out value="${sessionScope.volunteer.address }"></c:out></td>
                                <td><c:out value="${sessionScope.volunteer.notes }"></c:out></td>
                                    <%--<td><c:out value="${item.picture }"></c:out></td>--%>
                                <td><c:out value="${sessionScope.volunteer.date }"></c:out></td>
                                    <%--<td><img alt="" src="./${item.picture }" class="img1" height="30px"
                                        width="30px"></td>--%>
                                <td>
                                    <a href="../personal/modifyvolunteer.jsp" class="a2">修改</a>
                                </td>
                            </tr>
                        </table>
                    </c:when>
                    <c:otherwise>
                        <h4 style="margin-top: 80px;margin-left: 40px">没有志愿者信息录入记录</h4>
                    </c:otherwise>
                </c:choose>
